...
abalone_clean <- abalone |>   
  ...
  # renaming: pH and change in area 
...
# base layer: ggplot 
ggplot(data = abalone_clean, # using clean data frame
       aes(x = mean_ph, # x-axis: mean pH
           y = change_in_area)) + # y-axis: change in shell surface area/day
  # second layer: data points as observations
  geom_point(size = 3,
             stroke = 1,
             fill = "firebrick3",
             shape = 21)
...
`x`, `predicted`, `std.error`, `conf.low`, `conf.high`, `group` (not going to worry about 'group' right now) 
...
**It is a a 'ggeffects' output and a dataframe**  
...
**At pH = 8, the model predicts that change in abalone shell surface area (mm^2^ d^-1^) to be 5.36 [95% CI: 5.08, 5.64] mm^2^**
